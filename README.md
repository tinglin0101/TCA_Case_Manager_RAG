...
### 實測比較:TAIDE v2.0 vs GPT-4o-mini(同一份向量庫)

用同一份題庫(50 題)、**同一份向量庫**各跑一輪 `python -m pytest --e2e`。因為檢索端完全相同(兩者檢索命中率都 100%),**下列差異全部來自 LLM 生成本身**,不是檢索。

| 指標       | TAIDE v2.0(本地) | GPT-4o-mini(雲端) |
| ---------- | ----------------- | ------------------ |
| 檢索命中率 | 100%              | 100%               |
| 轉介率     | 6%                | 14%                |
| 平均覆蓋率 | 50%               | 57%                |
...
| 面向                 | TAIDE v2.0            | GPT-4o-mini |
| -------------------- | -------------------- | ----------- |
| 誤轉介(不該轉卻轉)   | 3 題                 | 7 題        |
| **漏轉(該轉卻沒轉)** | **1 題(E4 噎到急救)** | 0           |
| 漏附出處(有答沒引用) | 4 題                 | 0           |
| 覆蓋率不足(答非所問) | 4 題                 | 2 題        |

**怎麼讀這張表:**

- **轉介率低 ≠ 比較好。** TAIDE 只轉 6% 看似「不亂轉」,但它是靠**漏掉 E4「噎到、臉色發紫的急救」這種真正該轉的緊急題**換來的——這是上線前必須先修的安全問題。GPT 轉介率 14% 偏高是「寧可多轉」,但一題緊急都沒漏。
- **GPT 較穩、較貼題**:每題都附出處、覆蓋率較高;代價是把 7 題衛教單其實答得出來的問題也轉走(過度保守)。
- **TAIDE 的待修點**:4 題忘了附出處、答非所問較多,符合本地 8B 常見的 JSON 截斷/轉介判斷不穩;其中 Q4「吞嚥困難者適合吃堅果嗎?」被誤轉介,正是「特定食物可不可以吃」那個待團隊拍板的政策題。
- **共同弱點**:兩者的 Q6「吞嚥困難者可選哪些食物」覆蓋率都偏低(TAIDE 7% / GPT 9%)。因為檢索相同,這比較可能是**參考答案/切塊比對**的問題,值得單獨檢查,不是模型的錯。

> 這是單次 e2e 的快照數字,覆蓋率又是字元 bigram 粗篩,僅供**相對**比較與擋退步用;醫療正確性仍以個案管理師人工複核為準。
